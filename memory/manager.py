...
class MemoryManager:
    ...
    def __init__(self, root: str | Path = "memory") -> None:
        ...
        if not self.index_path.exists(): self._write_json(self.index_path,{"version":"0.9.0","encounters":0,"records":0})
    ...
    def _read_index(self):
        ...
        except (FileNotFoundError,json.JSONDecodeError): return {"version":"0.9.0","encounters":0,"records":0}
    ...
    def learned_facts(self,subject):
        ...
        for path in (self.organized_root/"knowledge").rglob("*.json"):
    ...
    def stats(self):
        ...
        return {"version":"0.9.0","root":str(self.root),"encounters":index.get("encounters",0),"records":index.get("records",0),"categories":{c:len(list((self.organized_root/c).glob("*.json"))) for c in self.CATEGORIES}}
