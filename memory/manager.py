...
class MemoryManager:
    ...
    def __init__(self, root: str | Path = "memory") -> None:
        ...
        if not self.index_path.exists(): self._write_json(self.index_path,{"version":"0.10.0","encounters":0,"records":0})
    ...
    def _read_index(self):
        ...
        except (FileNotFoundError,json.JSONDecodeError): return {"version":"0.10.0","encounters":0,"records":0}
    def _update_index(self,encounter_delta=0,record_delta=0):
        index=self._read_index(); index["version"]="0.9.0"; index["encounters"]=index.get("encounters",0)+encounter_delta; index["records"]=index.get("records",0)+record_delta; self._write_json(self.index_path,index)
    ...
    def remember(self,content,*,response="",source="encounter",metadata=None):
        ...
        raw={"version":"0.10.0","memory_id":encounter_id,"created_at":record["created_at"],"source":source,"content":content,"response":response,"organized_category":category,"metadata":metadata or {}}
    ...
    def stats(self):
        ...
        return {"version":"0.10.0","root":str(self.root),"encounters":index.get("encounters",0),"records":index.get("records",0),"categories":{c:len(list((self.organized_root/c).glob("*.json"))) for c in self.CATEGORIES}}
